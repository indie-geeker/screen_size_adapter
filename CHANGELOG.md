...
> **BREAKING:** 0.5.0 is the config-first, binding-level release. Apps should
> initialize with `ScreenSizeAdapterConfig`, write plain design-unit numbers in
> widgets, and use `ScreenSizeAdapterScope` for manually mounted non-primary
> views.
...
- `ScreenSizeAdapterScope` for corrected `MediaQuery` values in explicit
  `View` subtrees.
...
- `ScreenSizeWidgetsFlutterBinding.instance` typed accessor for multi-view
  integration code.
- Example app, package validation workflow, release-readiness checklist, and
  desktop multi-view verification checklist.

### Changed
- **BREAKING:** `ScreenSizeWidgetsFlutterBinding.ensureInitialized` now accepts
  a complete `ScreenSizeAdapterConfig` instead of a separate design size plus
  compatibility parameters.
- **BREAKING:** `attachView` and `updateView` now accept a complete
  `ScreenSizeAdapterConfig`, so each per-view registration is explicit and
  replaceable as a unit.
- **BREAKING:** Public view inspection accepts `FlutterView` rather than raw
  view IDs.
- **BREAKING:** Bare `num` extensions, `ScreenSizeHelper`, and the old widget
  wrapper API are removed from the public surface. Widget code should use plain
  numbers in design units.
- `ScreenSizeAdapterConfig.maxScale` defaults to `null` instead of `2.0`, so
  `ScaleAxis.width` keeps `MediaQuery.width == designSize.width` in both
  portrait and landscape unless an app opts into a cap.
- `ScreenSizeAdapterConfig.copyWith` can explicitly clear nullable scale bounds
  via `clearMinScale` and `clearMaxScale`.
- Minimum Flutter requirement is now `>=3.29.0`.
...
- Pointer packets are converted with the registered view's effective device
  pixel ratio.
- README initialization snippets install `ScreenSizeWidgetsFlutterBinding`
  before any other `WidgetsBinding` initialization.
- Dartdoc link warnings in public API comments.

### Migration
- Replace `ScreenSizeWidgetsFlutterBinding.ensureInitialized(size, config: ...)`
  with `ScreenSizeWidgetsFlutterBinding.ensureInitialized(config)`.
- Replace `attachView(view: view, designSize: ..., scaleAxis: ...)` with
  `attachView(view: view, config: ScreenSizeAdapterConfig(...))`.
- Replace `updateView(view: view, designSize: ...)` with a full replacement
  config via `updateView(view: view, config: current.copyWith(...))`.
- Replace `scaleForViewId(viewId)` / `configForViewId(viewId)` with
  `scaleForView(view)` / `configForView(view)`.
- Replace `100.dp`, `14.sp`, `0.5.sw`, and related extension-based sizing with
  plain numbers or normal `MediaQuery` expressions.
- If your app relied on the old `maxScale: 2.0` cap, pass
  `maxScale: 2.0` explicitly.
- For widget tests, wrap pumped content in `ScreenSizeTestEnvironment`; do not
  install the production binding inside `testWidgets`.
